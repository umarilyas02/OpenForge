...
## A `drizzle-kit migrate` deploy step fails partway through

Verified directly (2026-09-17, against a disposable Postgres 18
container, not just read from documentation): a failing migration is
**atomic per file** — drizzle-kit wraps each `packages/db/migrations/*.sql`
file in a single transaction. A deliberately broken two-statement test
migration (a valid `CREATE TABLE` followed by an invalid `ALTER TABLE`)
left **zero trace** on failure: the table from the first statement was
rolled back, and `drizzle.__drizzle_migrations` gained no row for the
failed migration. The database was left in exactly its prior, fully-valid
state — no partial schema, no stuck tracking row.

Recovery is therefore just: fix the migration file, run `drizzle-kit
migrate` again (`corepack pnpm --filter @openforge/db run migrate`).
It re-applies starting from the first migration `__drizzle_migrations`
doesn't already have a row for — confirmed the same way, re-running after
the fix applied cleanly and the tracking table showed the newly-succeeded
migration with no duplicate or gap.

**What this does not cover**: a migration that succeeds at the database
level but does something the application code doesn't expect (a
semantic bug, not a transaction failure) — that needs a real down
migration or a forward-fixing migration, and `drizzle-kit` in this repo
generates forward-only migrations (no autogenerated down/rollback SQL).
