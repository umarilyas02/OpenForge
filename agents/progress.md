...
- Objective (2026-09-17, same day continued): operational runbooks (Phase
  ...
  - Verified: full repo lint (26/26) and `packages/workspace`'s test
    ...
    themselves) pass.
- Objective (2026-09-17, same day continued): migration recovery (Phase
  6.1) — tested empirically instead of trusting documentation.
  - Completed: spun up a disposable Postgres 18 container, applied the
    real 3 migrations, then crafted a deliberately broken 4th migration
    (a valid `CREATE TABLE` followed by an invalid `ALTER TABLE`) and ran
    `drizzle-kit migrate` against it directly. Confirmed the failure is
    atomic per migration file: the table from the valid first statement
    was rolled back along with the broken second one, and
    `drizzle.__drizzle_migrations` gained no row for it — the database
    was left in exactly its prior valid state, not a partially-applied
    one. Fixed the migration and re-ran: applied cleanly, tracking table
    showed the new migration with no gap or duplicate. Documented as a
    new runbook in `docs/cms-operational-runbooks.md`, including the
...
- [x] Migration recovery.
  - Evidence: verified directly against a disposable Postgres 18
    container, 2026-09-17, not assumed from documentation — a
    deliberately broken two-statement migration left zero trace on
    failure (the valid first statement rolled back with the broken
    second one; no tracking row written), and re-running after a fix
    applied cleanly with no gap or duplicate. See
    `docs/cms-operational-runbooks.md`'s migration runbook. Confirmed
    limit: this only covers transaction-level failure, not a migration
    that succeeds but is semantically wrong — this repo's migrations are
    forward-only, no autogenerated down migration.
